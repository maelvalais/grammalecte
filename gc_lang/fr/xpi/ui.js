...
function getGCResultPromise (sParagraph, iParagraph, sLang, bDebug) {
    // For some reason, you can’t use result of PromiseWorker within a Task,
    // you have to wrap it in a common Promise.
    return new Promise(function (resolve, reject) {
        let xPromise = xGCEWorker.post('parseAndGenerateParagraph', [sParagraph, iParagraph, sLang, bDebug]);
        xPromise.then(
            function (aVal) {
                resolve(aVal);
            },
            function (aReason) {
                // rejected
                resolve('<p class="message">' + aReason + '</p>');
            }
        ).catch(
            function (aCaught) {
                // error
...
function sendTextToPanel (sText) {
    ...
    Task.spawn(function* () {
        ...
        for (let sParagraph of text.getParagraph(sText)) {
            if (sParagraph.trim() !== "") {
                sRes = yield getGCResultPromise(sParagraph, iParagraph, "FR", false);
        ...
        xGCPanel.port.emit("addElem", '<p class="message">' + _("numberOfParagraphs") + " " + res + '</p>');
    }, function (e) {
...
function getLxgResultPromise (sParagraph) {
    // For some reason, you can’t use result of PromiseWorker within a Task,
    // you have to wrap it in a common Promise.
    return new Promise(function (resolve, reject) {
        let xPromise = xGCEWorker.post('analyzeWords', [sParagraph]);
        xPromise.then(
            function (aVal) {
                resolve(aVal);
            },
...
function analyzeWords (sText) {
    ...
    Task.spawn(function* () {
        ...
        for (let sParagraph of text.getParagraph(sText)) {
            if (sParagraph.trim() !== "") {
                sRes = yield getLxgResultPromise(sParagraph);
        ...
        xLxgPanel.port.emit("addElem", '<p class="message">' + _("numberOfParagraphs") + " " + res + '</p>');
    }, function (e) {
...
function createTestPanel () {
    if (xTestPanel === null) {
        ...
        xTestPanel.port.on("checkText", function (sText) {
            ...
            let xPromise = xGCEWorker.post('parse', [sText, "FR", true]);
            xPromise.then(
                function (aVal) {
