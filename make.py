...
def createOXT (spLang, dVars, dOxt, spPackage, bInstall):
    ...
    # Package and parser
    copyGrammalectePackageInZipFile(hZip, spPackage, dVars['binary_dic'])
...
def createXPI (sLang, dOptData, bLaunchFx=False):
...
def create (sLang, xConfig, bInstallOXT, bJavaScript, bLaunchFx):
    ...
    for sf in os.listdir("gc_core/py"):
        if not os.path.isdir("gc_core/py/"+sf):
            open(spPack+"/"+sf, "w", encoding="utf-8").write(fileFile("gc_core/py/"+sf, dVars))
    print("+ Modules: ", end="")
    for sf in os.listdir(spLang+"/modules"):
        if not sf.endswith(".tpl.py") and not sf.startswith("gce_") and sf != "gc_tmp_eval.py":
            file_util.copy_file(spLang+"/modules/"+sf, spPack)
            print(sf, end=", ")
    print()
    ...
    createOXT(spLang, dVars, xConfig._sections['oxt'], spPack, bInstallOXT)
    ...
    if bJavaScript:
        ...
        createXPI(sLang, dOptData, bLaunchFx)

    return dVars['version']


def copyGrammalectePackageInZipFile (hZip, spPack, sDicName):
    for sf in os.listdir("grammalecte"):
        if not os.path.isdir("grammalecte/"+sf):
            hZip.write("grammalecte/"+sf, "pythonpath/grammalecte/"+sf)
    for sf in os.listdir(spPack):
        if not os.path.isdir(spPack+"/"+sf):
            hZip.write(spPack+"/"+sf, "pythonpath/"+spPack+"/"+sf)
    hZip.write("grammalecte/_dictionaries/"+sDicName, "pythonpath/grammalecte/_dictionaries/"+sDicName)
...
def quick_test():
    ...
    f.write(json.dumps(compile_rules.regex2js(r"\([a-zA-Z]\)")))
    f.close()
